...
class DbTemplate extends Form {
	...
	*/
	...
	function CreateDataListBank($myID, $myWhere, $myData, $order = "") {
		global $db;
        ...
        if ($myID != "") {
            $sql = "SELECT * FROM " . $this->table . $SqlWhere . $order ;
            $Query = $db->Query($sql) OR DIE("Error: เกิดความผิดพลาด <br>$sql<br>\n");
            $RecordCount = $db->NumRows($Query);
            if ($RecordCount > 0) {
                $index = 1;
                while ($index <= $RecordCount) {
                    ...
                    echo ">";
                    echo $RowGroup["bank_name"] . " " . $RowGroup["bank_acc"] . " (" . $RowGroup["bank_number"] . ")";
                    echo "</option>";
                    $index++;
                }
            }
	...
	function getDataDescFirstID($myID, $myWhere="") {
		global $db;
        ...
        if ($myID != "") {
            $sql = "SELECT $myID FROM " . $this->table . $SqlWhere . " ORDER BY id ASC LIMIT 1";
            $Query = $db->Query($sql);
            $RecordCount = $db->NumRows($Query);
